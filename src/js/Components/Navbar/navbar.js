...
import { LoginContext } from '../../js/js/Logincontext';
import Sharemodal from '../../js/sharemodal';
var FileSaver = require('file-saver');
import { IsAuto } from '../../js/js/Isauto';
import { updateContext } from '../../js/js/updatecontext';
import { AnonContext } from '../../js/js/AnonContext';
...
export default function Navbar({ toggleTheme, isDark, text, toggleViewNotes, setText }) {
  ...
  const LogOut = () => {
    fetch("https://api.wordpad.app/logout", {
      method: 'GET',
      headers: {
        Accept: '*/*',
        token: loginToken.current,
                ...
                <Popover.Body className={isDark ? "tooltipdark" : "tooltiplight"} >
                  <div className='exporttool' >
                    <div style={{ width: "40%", display: "flex", justifyContent: "center", alignItems: 'center' }}>
                      <input type="radio" name="export" onClick={pdf} value="pdf" /><label style={{ paddingLeft: '10%' }}>PDF</label>
                    </div>

                    <div style={{ width: "50%", display: "flex", justifyContent: "center", alignItems: 'center' }}>
                      <input type="radio" name="export" value="word" onClick={wordFile} />
